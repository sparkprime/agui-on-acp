...
> This is a mapping doc, not a design rationale doc — it documents the
> field-level correspondence as it stands today. For *why* the bridge is
> built this way (a genuinely native AG-UI run endpoint carrying ACP's
> domain vocabulary through AG-UI's own extension points, plus a
> separate, honestly-non-AG-UI session-bootstrapping envelope), see
> `DESIGN.md`'s "Why this exists" section. Every row below that says
> "State held" or "Not 1:1" is a place where that native-ness had to be
> earned deliberately — either because AG-UI genuinely has no equivalent
> concept (permissions, sessions), or because an earlier implementation
> took a shortcut that turned out to violate AG-UI's own contract (see
> the `proposals/` directory for the three such fixes: permission
> `responseSchema`, `state`-based config, and plan/usage/sessionInfo as
> `state`).
...
| **Connect** | `GET /ag-ui/sessions/{id}/connect` | `session/load` | Replays the existing transcript as a `STATE_SNAPSHOT` (folded plan/usage/sessionInfo/configOptions state) then a `MESSAGES_SNAPSHOT`. Spawns a short-lived subprocess for the replay; the bridge's `cwd` record (see Persistent State) supplies `cwd` — the client doesn't resend it. |
...
| `ToolCallProgress` (`status=running`, with `raw_output`, no `raw_input`) | (dropped — no AG-UI event) | ACP carries intermediate/in-flight output under the same `tool_call_update` kind while a tool is still running (distinct from the final `raw_output` at `completed`/`failed`, which *does* map to `TOOL_CALL_RESULT` below). AG-UI has no first-class "tool progress" event, and the bridge does not currently repurpose any existing event for it — `_process_tool_call_update` only handles `raw_input is not None` or a terminal `status`, so an in-progress-with-`raw_output`-only update falls through as a no-op. Live in-flight tool output is simply not surfaced today. See "Would need minor extensions to ACP or AG-UI" below. |
...
| **ACP 0.11:** `ConfigOptionUpdate` | `STATE_DELTA` (`[{op:"add", path:"/configOptions", value:[…]}]`) | The notification carries the full set, so this is a full-value update of the `/configOptions` path — but as a JSON Patch `STATE_DELTA`, not a partial `STATE_SNAPSHOT`, so it doesn't wipe the `plans`/`usage`/`sessionInfo` the client is also holding (the "STATE_SNAPSHOT merge trap"). Also refreshes `ActiveSession.config_options` (via `on_config_options_changed`) so the next run's `state.configOptions` diff is stable against agent-driven changes. **`add` not `replace`:** the AG-UI reference client applies patches via `fast-json-patch.applyPatch(state, delta, /*validate=*/true, …)`, which **silently drops** `replace` on a missing path — so `add` (RFC 6902 §4.1: create-or-replace for object members) is used for *every* `STATE_DELTA` op so the first update for a path the client's baseline didn't pre-populate still lands. |
| **ACP 0.11:** `UsageUpdate` | `STATE_DELTA` (`[{op:"add", path:"/usage", value:{used, size, cost?}}]`) | `cost` (when present) is `{amount, currency}`. A running counter — exactly what AG-UI `state` is for (not a fire-and-forget `CUSTOM`). **State held:** `_usage` on the bridge, so the run-start `STATE_SNAPSHOT` baseline carries the current value (persists across runs within a session and survives reconnect via the replay accumulator). |
| **ACP 0.11:** `SessionInfoUpdate` | `STATE_DELTA` (`[{op:"add", path:"/sessionInfo", value:{title, updatedAt}}]`) | A title/timestamp pair with explicit "set to null to clear" semantics — a current value, not a notification. `null` `title`/`updatedAt` are preserved (not dropped) so the "cleared" signal reaches the client. **State held:** `_session_info` on the bridge. |
| **ACP 0.11:** `AgentPlanUpdate` | `STATE_DELTA` (`[{op:"add", path:"/plans/default", value:{type:"items", entries:[…]}}]`) | Legacy single-plan full-replace. Stored under the sentinel key `"default"` (no ACP-provided id). `STATE_DELTA`/`STATE_SNAPSHOT` semantics already — ACP's own docstring says "the client replaces the entire plan with each update." **State held:** `_plans["default"]` on the bridge. |
| **ACP 0.11:** `AgentPlanContentUpdate` | `STATE_DELTA` (`[{op:"add", path:"/plans/<id>", value:{…}}]`) | Newer multi-plan variant; keyed by the plan's `id`. The value mirrors the discriminated union (`items`/`file`/`markdown`), not assuming a checklist. **State held:** `_plans[id]`. |
...
|---|---|---|
| Server → client (advertise options) | `STATE_SNAPSHOT` with `modes` / `models` / `currentModeId` / `configOptions` / `plans` / `usage` / `sessionInfo` | Read at Create time from `session/new`'s response, stashed on `ActiveSession`, then emitted as a snapshot after `start_run` attaches the run's queue. `plans`/`usage`/`sessionInfo` are bridge-tracked evolving state (persist across runs within a session, included even when empty so the paths the mid-turn `STATE_DELTA` `add` ops touch exist from the first run). Re-emitted on `ConfigOptionUpdate` notifications mid-turn (as a `STATE_DELTA`, not a snapshot). |
...
Replay is **not** a second state machine. The live state machine is the only
implementation of the sequencing rules; during `session/load` replay `_emit()`
folds each event into a single `MessageSnapshotAccumulator` (both the
message list and, since the plan/usage/sessionInfo change, a parallel
`state` dict) instead of putting it on the SSE queue, and `end_replay()`
emits one `STATE_SNAPSHOT` then one `MESSAGES_SNAPSHOT` from the
accumulator. A future sequencing fix in the live handlers automatically
applies to replay too, since there is nothing replay-specific left to
keep in sync.
...
| `_plans: dict[str, Any]`, `_usage`, `_session_info` (bridge) | session-level, persists across runs | Evolving `state` extras (plan/usage/sessionInfo) tracked on the bridge itself so `extra_state()` can pre-populate the run-start `STATE_SNAPSHOT` baseline — keeping mid-turn `STATE_DELTA` `add` ops valid under a strict RFC 6902 applier. Updated by the `session/update` handlers, not reset by `start_run`/`attach_resume_queue`. |
...
| Tool progress (in-flight output) | ❌ (no event today) | ✅ `ToolCallProgress` w/ `status=running` | **not implemented** — falls through as a no-op (see the `session/update` table above); not the same as "repurposes `TOOL_CALL_ARGS`", which was never actually shipped |
...
| Tool progress as a first-class event | AG-UI has no `TOOL_CALL_PROGRESS`; the bridge currently drops in-flight `raw_output` entirely (no event emitted — see the `session/update` mapping table above) | An AG-UI spec addition for in-flight tool output, or a `CUSTOM` event with a stable name. |
...
| Multiple sessions multiplexed on one ACP subprocess | ACP is designed for this (one process, one `Map<sessionId, Info>`); this bridge deliberately runs one subprocess per session for simplicity. Reusing one connection across many sessions is a deeper architectural change — out of scope for the current model. |
