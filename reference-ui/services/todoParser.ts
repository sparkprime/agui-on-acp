 ...
 * # Task: Task Title
 *
 * <!-- acp-task:meta {"id":"task-xxx","status":"in_progress","timeSpent":3600000} -->
 *
 * ## TODO
 * - [ ] Uncompleted item
 * - [x] Completed item
...
export function parseTodoMd(content: string): ParsedTodo | null {
  try {
    ...
    const metadataMatch = content.match(/<!--\s*acp-task:meta\s+({.*?})\s*-->/);
    if (metadataMatch) {
      try {
...
---
*Managed by ACP Agent - Cross-tool task management*
`;
}
