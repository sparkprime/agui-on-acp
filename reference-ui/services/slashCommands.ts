...
export const slashCommands: Record<string, SlashCommand> = {
  ...
  skills: {
    ...
    execute: async (args, context) => {
      // Stubbed out - skills will be loaded from agent config in the future
      const skills: Array<{name: string, path: string, token_count?: number}> = [];

      let skillsList = '## Skills\n\n';

      if (skills.length === 0) {
        skillsList += 'No skills found.\n\n';
        skillsList += 'Skills can be configured via the agent settings.';
      } else {
  ...
  plugin: {
    name: 'plugin',
    description: 'List configured MCP plugins/servers',
    aliases: ['plugins', 'mcp'],
    execute: async (args, context) => {
      // Stubbed out - MCP servers will be managed via REST API in the future
      const servers: Array<{
        name: string;
        command?: string;
        args?: string[];
      ...
      if (servers.length === 0) {
        pluginList += 'No MCP servers configured.\n\n';
        pluginList += 'MCP server management will be available via the Config panel.';
      } else {
