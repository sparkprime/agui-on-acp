...
export function getTodoFilePath(projectDir: string): string {
  return `${projectDir}/.acp-agui/TODO.md`;
}

/**
 * Ensure the .acp-agui directory exists
 */
async function ensureDataDir(projectDir: string): Promise<void> {
  const dirPath = `${projectDir}/.acp-agui`;
  try {
    await createDirectory(dirPath);
  } catch (error) {
    // Directory might already exist, which is OK
    console.log('[taskStore] .acp-agui directory check:', error);
  }
}
...
export async function createTaskSession(
  ...
  // Ensure .acp-agui directory exists
